...
%LVR = (%LVAVG1 − %LVAVG2 / %LVAVG1 × 100where:%LVR = Percent leaking valve reduction.%LVAVG1 = (%VLi + %VLi = 1)/2.%LVAVG2 = (%VLi = 1 + %VLi = 2)/2.where:%VLi, %VLi = 1, %VLi = 2 are percent leaking valves calculated for subsequent monitoring periods, i, i + 1, i + 2.
...
![](/graphics/er22ap94.293.gif)%RR = percent reduction required each quarter, as calculated according to § 63.168(e)%VL = percent leaking valves, calculated according to § 63.168(e), at the time elected to use provisions of § 63.168(d)(1)(ii)
