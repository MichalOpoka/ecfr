...
![](/graphics/er17no16.067.gif)Where:HVi = the heating value for FMPi, in Btu/scfHVi,j = the heating value for FMPi, for partial month j, in Btu/scfVi,j = the volume measured by FMPi, for partial month j, in Btu/scfSubscript i represents each FMP for the lease, unit PA, or CASubscript j represents a partial month for which heating value HVi,j is effectivem = the number of different heating values in a reporting month for an FMP
