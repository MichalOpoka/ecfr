...
PM tons = (EFbase − EFcont) × (PR) × (UL) × (LF) × (10−6)Where:EFbase = deteriorated baseline PM emission rate (g/kW-hr).EFcont = deteriorated controlled PM emission rate (g/kW-hr).PR = maximum engine power for the engine (kW).UL = useful life (hr).LF = the load factor that would apply for your engine under § 1042.705.