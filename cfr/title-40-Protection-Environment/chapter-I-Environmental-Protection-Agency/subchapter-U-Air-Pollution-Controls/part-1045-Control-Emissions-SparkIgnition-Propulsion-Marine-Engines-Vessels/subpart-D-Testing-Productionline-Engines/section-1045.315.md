...
Ci = Max [0 or Ci−1 + Xi − (STD + 0.25 × σ)]Where:Ci = The current CumSum statistic.Ci−1 = The previous CumSum statistic. For the first test, the CumSum statistic is 0 (i.e., C1 = 0).Xi = The current emission test result for an individual engine.STD = Emission standard (or family emission limit, if applicable).
