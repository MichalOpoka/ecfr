...
Path-loss (L) = Σi P(i) \* Li = PLOS \* LLOS + PNLOS \* LNLOS,where PLOS is the probability of line-of-sight, LLOS is the line-of-sight path loss, PNLOS is the probability of non-line-of sight, LNLOS is the non-line-of-sight path loss, and L is the combined path loss. The WINNER II path loss models include a formula to determine PLOS as a function of antenna heights and distance. PNLOS is equal to (1−PLOS). In all cases, the AFC system will use the correct WINNER II parameters to match the morphology of the path between a standard power access point and a fixed microwave receiver (*i.e.,* Urban, Suburban, or Rural).
